 ...
 *
 * @tparam T type of array | T 代表的是数组的类型
 * @param [in,out] arr Array to be sorted | arr代表的是待排序的数组
 * @param n Size of Array | n 代表的是数组的长度
 * @brief 假设我们以 3 2 1 4 5 为例 第一轮过后为 2 3 1 4 5 (交换了2,3) 第二轮过后为 1 2 3 4 5(交换了 3,1 接着交换了 2,1)
 */
template <typename T>
...
void insertionSort(std::vector<T> *arr) {
    ...
    for (size_t i = 1; i < n; i++) {
        T temp = arr[0][i]; // 这里的arr[0] 相当于 *arr
        int32_t j = i - 1;
